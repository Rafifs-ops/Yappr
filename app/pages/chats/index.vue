...
<template>
    <main class="h-full flex flex-col space-y-4">
        <!-- Header -->
        <div class="flex items-center justify-between bg-black/40 backdrop-blur-md p-4 rounded-xl border border-white/10 shadow-lg">
            <h1 class="text-2xl font-bold text-white tracking-wide">Chats</h1>
            <button @click="showNewChatModal = true" class="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg font-medium transition-all duration-300 shadow-[0_0_15px_rgba(37,99,235,0.5)]">
                <Icon name="mdi:plus" class="w-5 h-5 inline-block mr-1" /> New Chat
            </button>
        </div>
        ...
        <div v-else-if="chats.length === 0" class="flex flex-col items-center justify-center py-16 bg-black/20 rounded-xl border border-white/5">
            ...
            <NuxtLink v-for="chat in chats" :key="chat._id" :to="`/chats/${chat._id}`" 
                ...
                <div class="ml-4 flex-1">
                    ...
                    <p class="text-gray-400 text-sm truncate mt-1 max-w-[200px] sm:max-w-md">
                        <span v-if="chat.latestMessage?.sender" class="font-medium text-gray-300">{{ chat.latestMessage.sender }}: </span>
        ...
        <!-- Modal New Chat -->
        <div v-if="showNewChatModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div class="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-md shadow-2xl flex flex-col max-h-[80vh]">
