...
<template>
    <main class="h-full flex flex-col bg-black/40 backdrop-blur-md rounded-2xl border border-white/10 overflow-hidden" style="height: calc(100vh - 180px);">
        <!-- Header -->
        <div class="flex items-center p-4 border-b border-white/10 bg-black/60 shadow-md">
            <NuxtLink to="/chats" class="text-gray-400 hover:text-white transition-colors mr-3 p-1 rounded-full hover:bg-white/10">
        ...
        <div ref="messagesContainer" class="flex-1 overflow-y-auto p-4 space-y-4 scroll-smooth">
            ...
            <div v-for="msg in messages" :key="msg._id" 
                ...
                <div class="flex items-end gap-2 max-w-[80%]">
                    ...
                    <div class="flex flex-col" :class="[msg.senderId?._id === authStore.session?.id ? 'items-end' : 'items-start']">
                        <span v-if="msg.senderId?._id !== authStore.session?.id" class="text-xs text-gray-400 mb-1 ml-1">{{ msg.senderId?.username }}</span>
